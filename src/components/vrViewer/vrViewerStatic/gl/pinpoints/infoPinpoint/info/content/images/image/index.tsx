...
const Image = ({image}:{image:TexturedImageType}) => {
    const {pinpoint, headerSize, gap, descriptionSize} = useContext(InfoPinpointContext)
    ...
    return (  
        <>
            <mesh
                position={[
                    -pinpoint.info.width/2 + (width/2) + offset + (image.index * 15),
                    (pinpoint.info.height/2) - (height/2) - headerYOffset - descriptionYOffset,
                    0
                ]}
                geometry={geometry}
                material={material}
