...
const PinpointsUi = ({transition}:{transition:boolean}) => {
    ...
    const {selectedScene, currentView, isAutoplay, camera, mainMeshRef, isEditorMode, setEnableOrbitControl} = useContext(VrViewerStaticContext)
    ...
    const handleDragPin = (e:MouseEvent)=>{
        if(isMouseDown && isEdit.type === 'dragPin'){
            ...
            const rotatedIntersectPosition = intersectPosition.applyQuaternion(quaternion)
            ...
            currentView.pinpoints = currentView.pinpoints.map(p=>{
                if(p._id === isEdit.pinId){
                    return {...p,
                        position:{
                            x:rotatedIntersectPosition.x,
                            y:rotatedIntersectPosition.y,
                            z:rotatedIntersectPosition.z 
                        }
                    }
                }
                else{
