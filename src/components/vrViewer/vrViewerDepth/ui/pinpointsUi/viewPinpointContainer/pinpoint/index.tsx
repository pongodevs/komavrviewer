...
const Pinpoint = () => {
    ...
    const {
        currentView, parallaxTeleport, selectedScene, isEditorMode, setCurrentView, 
                        ...
                        if(!isDragged && draggedView._id === ''){
                            if(pin.redirectUrl){
                                ...
                                if(nextProject){
                                    ...
                                    if(nextView){
                                        setOpacity(0)
                                        parallaxTeleport(nextView, selectedProject.globalSettings.transition.duration)
                                    }
                                ...
                                const findView = selectedScene.viewList.find(v=>v.viewName === pin.toViewName) as ViewListType
                                if(findView){
                                    // Change map
                                    ...
                                    if(findMap){
                                        ...
                                        // Change pin
                                        const findPin =findMap.pinpoints.find(pin=>pin.toViewName === findView.viewName)
                                        if(findPin){
                ...
                >
                    ...
                    {(pin.thumbnailUrl !== '' && progress === 0) && findCustomPinpoint?.showThumbnail?
                        <img
                            ...
                            src={selectedScene.viewList.find(view=>{return view.viewName === pin.toViewName})?.imageUrl4000 || pin.thumbnailUrl}
                        />
                    :null}
                    {/* Upload Thumbnail */}
