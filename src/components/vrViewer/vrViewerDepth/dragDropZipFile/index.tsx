    ...
    useEffect(()=>{
        const myInterval = async()=>{
            try{
                ...
                const vrProject = JSON.parse(json)
                if(vrProject.scenes.length > 0){
                    const sceneLength = vrProject.scenes.length 
                    const viewLength = vrProject.viewList.length
                ...
                <DragAndDropOverlay
                    ...
                    onDrop={(e:any)=>{
                        ...
                        if(file.size > 1000000000){
                            return toast.error('File size too large, maximum size only 1 GB.')
                        }
                        if(file.type !== 'application/x-zip-compressed'){
                            return toast.error('File must be zip.')
                        }
                        const storageRef = ref(storage, `pongovr/projects/${projectId}/${dateNow}/${projectId}-depth.zip`)
